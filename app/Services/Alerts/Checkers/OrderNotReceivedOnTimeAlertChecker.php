...
class OrderNotReceivedOnTimeAlertChecker
{
    ...
    public function check(): void
    {
        ...
        foreach ($orders as $order) {
            ...
            if ($expectedDate->gte($today) || $remaining <= 0) {
                $this->alertService->resolve(
                    AlertType::ORDER_NOT_RECEIVED_ON_TIME,
                    'commande',
                    $order->id
                );
                continue;
            ...
            $this->alertService->createOrUpdate([
                'type' => AlertType::ORDER_NOT_RECEIVED_ON_TIME,
                'title' => 'Commande non réceptionnée à temps',
                'message' => "La commande {$reference} n'a pas été totalement réceptionnée à la date prévue.",
                'severity' => $severity,
                ...
                'metadata' => [
                    ...
                    'contrat_id' => $order->contrat_id ?? null,
                    'entrepot_id' => $order->entrepot_id ?? null,
                ],
            ]);
        }
    }
